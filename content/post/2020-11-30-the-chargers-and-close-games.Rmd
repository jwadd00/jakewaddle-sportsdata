...
Equipped with several years of final scores from the NFL, we can start to perform transformations on the data and start asking questions. As noted in the introduction, we're interested in capturing and measuring close game frequency. We're also going to take a look at win percentage in closely contested games. 

We'll use `case_when()` to establish a rule that defines a close game as a game in which the final score differential is less than eight. To measure win percentage, we can create a column that annotates a team's win. 
...
transform <- all_scores %>%
  ...
  mutate(count_games = n(),
         count_close_games = sum(within_a_score))  %>% 
  ungroup()
```
...
As expected, the Chargers lead the way with close games. 


```{r Close Game Count, warning = FALSE, message = FALSE}
transform %>% 
  distinct(team, count_close_games) %>% 
  arrange(desc(count_close_games)) %>% 
  slice(1:10) %>% 
  rename(Team = team,
         `Number of Close Games` = count_close_games) %>% 
  kable(format = 'html') %>% 
  kable_styling()
```


# Win Percentage in Closely Contested Games

notes here


```{r Close Game Win Pct, warning = FALSE, message = FALSE}
transform %>% 
  slice(1:10) %>% 
  kable(format = 'html') %>% 
  kable_styling()
